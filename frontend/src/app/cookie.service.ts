...
export class CookieService {
  ...
  public deleteCookie(cookieName: string) {
    this.setCookie({ name: cookieName, value: '', expireSecs: -1 });
  }
   ...
   * {service instance}.setCookie({name:'niceCar', value:'red', expireSecs:10 }); <- For all this examples if path is not provided default will be root
   */
  public setCookie(params: any) {
    let d: Date = new Date();
    d.setSeconds( d.getSeconds() + (params.expireSecs ? params.expireSecs : 300))
